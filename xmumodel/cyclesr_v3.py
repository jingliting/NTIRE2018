...
class CycleSR(Model):
    ...
    def generator(self):
        ...
        self.norm_input_a = utils.normalize_color_tf(self.input_a)
        self.norm_input_b = utils.normalize_color_tf(self.input_b)

        x = tl.InputLayer(self.norm_input_a, name='a_input')
        ...
        self.fake_b = tf.clip_by_value(x.outputs, 0.0, 1.0)

        y = tl.InputLayer(self.fake_b, name='b_input')
        ...
        self.fake_a = tf.clip_by_value(y.outputs, 0.0, 1.0)
    ...
    def cacuLoss(self):
        ...
        self.loss_b = self.cycle_consistency_loss(self.norm_input_b, self.fake_b)  # HR loss

        PSNR = utils.psnr_tf(self.norm_input_b, self.fake_b, is_norm=True)

        # Scalar to keep track for loss
        summary_loss = tf.summary.scalar("loss_b", self.loss_b)
        summary_psnr = tf.summary.scalar("PSNR", PSNR)

        streaming_loss, self.streaming_loss_update = tf.contrib.metrics.streaming_mean(self.loss_b)
    ...
    def train(self,batch_size= 10, iterations=1000, lr_init=1e-4, lr_decay=0.5, decay_every=2e5,
        ...
        train_a_op = optimizer.minimize(self.loss_a)
        train_b_op = optimizer.minimize(self.loss_b)
        ...
        with self.sess as sess:
            ...
            for i in tqdm(range(iterations)):
                ...
                summary,_,_ = sess.run([self.train_merge, train_a_op, train_b_op], feed)
    ...
    def predict_a(self, x):
        return self.sess.run(self.fake_a, feed_dict={self.input_b: x})

    def predict_b(self, x):
        return self.sess.run(self.fake_b, feed_dict={self.input_a: x})
